---
title: "interactive_markdown_for_my_analysis"
...
# Three drugs: MTX (for MitCISntrone), CIS (for CISliplatin), CIS (for Cisplatin).
...
# If you want to evaluate the effect of a certain drug over another drug, such as Cisplatin vs Oxaliplatin or/and Cisplatin vs Mitoxantrone, at a specific timpoint (3h for example)
...
# If I want to evaluate the effect of time for a given drug.
...
## If I want to investigate the overlap of up-reg pathways after a certain timepoint, across each treatment.
...
## 3. Transcription factor activity inference using decoupleR and ROMA (Representation of module activity from omics data).
...
# Through this example, I will show you how you can generate side-by-side plots showing the activity vs expression of the top 50 overdispersed TFs, in your drug of interest, over time. Here, I am not be specific to a given TF, but with the code below, the user has to choice to plot any TFs, not specifically the top 50 overdispersed TFs.
...
# 1st approach: you ran rRoma, using your normalized gene expression matrix, and the gene sets from GTRD (that you can retrieve from MSigDB), you then saved the output (using the saveRDS function).
...
# One thing to do just after you loaded your roma results, is to ensure that the TF names in the sample matrix (i.e. rownames) start with a capital letter, then the rest is in lowercase.
...
# 1. An input prior knowledge network, typically, you import from decoupleR the GRN CollecTRI (from decoupleR), available for 3 species: human, mouse, rat.
...
# counts_decoupleR = matrix object with genes as rows and samples as columns, and normalized (with the method of your choice)
...
# 2nd option: Among the TFs you decided to test, such as those from GTRD (with roma) or those from CollecTRI (with decoupleR), I am only interested in specific TFs, for which I want to examine the activity vs expression, over time, in samples treated with a specific drug. 
...
get_tfs_interest_activity_expression <- function(tf_results, gene_expression_df, TF_vector){
  
  
  # Get activity scores of the TFs of interest
  activity_tfs_interest <- tf_results %>% as.data.frame() %>% filter(rownames(.) %in% TF_vector) 
  
  # Get expression values of the TFs of interest
  expression_tfs_interest <- gene_expression_df %>% filter(rownames(.) %in% TF_vector) 
  
  # Get the vector of overdispersed TFs for which we can retrieve both their activity and expression
...
# Create a function which retrieves the mean expression and activity of the TFs of interest, over time, in the samples treated with the drug of interest.
get_longitudinal_mean_expression_activity_tfs_drug_interest <- function(tf_results, gene_expression_df, TF_vector, timepoint_vector, drug_interest, time_info){
  
  
  # Call function defined above to retrieve vector of TFs for which I retrieve both activity and expression  
  tfs_interest_activity_expression <- get_tfs_interest_activity_expression(tf_results, gene_expression_df, TF_vector)
  
  
  # Restrict sample and gene expression matrices to this vector of TFs, and put it in the same order in both matrices (+ convert matrices into df)
  activity_scores_tfs_interest_df <- tf_results[tfs_interest_activity_expression,] %>% as.data.frame()
  expression_values_tfs_interest_df <- gene_expression_df[tfs_interest_activity_expression,]
  
  
  # Select, from both df columns, samples treated with the drug of interest
  ...
  # 1. Get the longitudinal mean expression of the TFs of interest, in the samples treated with the drug of interest
  ...
  # 2. Get the longitudinal mean activity of the TFs of interest, in the samples treated with the drug of interest
...
plot_longitudinal_mean_expression_activity_drug_interest <- function(tf_results, gene_expression_df, TF_vector, timepoint_vector, drug_interest, n_timepoints, time_info){
  
  
  # Call the function defined above to retrieve the longitudinal mean expression and activity of the TFs of interest  
  longitudinal_mean_expression_activity_tfs_drug_interest <- get_longitudinal_mean_expression_activity_tfs_drug_interest(tf_results, gene_expression_df, TF_vector, timepoint_vector, drug_interest, time_info)
  
  
  # Store the expression and activity dfs (from the call output) into new variables
  ...
  # Loop through the TFs of interest for which we were able to retrieve both the expression and activity
  ...
  longitudinal_mean_expression_tf_plot <- ggplot(longitudinal_mean_expression_tfs_drug_interest[i:(i+4),] %>% arrange(as.numeric(timepoint)), mapping = aes(x = factor(timepoint, levels=c("3","6","12","18","24")), y = Mean_Expression)) +
  ...
  longitudinal_mean_activity_tf_plot <- ggplot(longitudinal_mean_activity_tfs_drug_interest[i:(i+4),] %>% arrange(as.numeric(timepoint)), mapping = aes(x = factor(timepoint, levels=c("3","6","12","18","24")), y = Mean_Activity)) +
    geom_point(size = 2, shape=21, aes(color = TF, fill=TF)) +
    ggrepel::geom_label_repel(aes(label=TF, fill=TF), label.size = 0.15, label.padding = 0.10) +
    geom_path(aes(color=TF, group=TF), size=1.1) +
    scale_fill_brewer(palette = "Set2") +
...
graphics.off()
all_plots <- plot_longitudinal_mean_expression_activity_drug_interest(tf_results, gene_expression_df, TF_vector, timepoint_vector, drug_interest, n_timepoints, time_info)
...
# How to call the different functions to retrieve, at the end, the plots of interest ?
...
roma_results <- readRDS(filename)
tf_results <- roma_results$SampleMatrix 
...
rownames(tf_results) <- rownames(tf_results) %>% str_to_title() %>% gsub("_target_genes","",.)
...
# In the case, where you have samples (i.e. column names) with no timepoint, the user needs to rename those columns, by adding 0h or 0d, in the column names.
# gene_expression_df_filename <- "...csv"
gene_expression_df <- read.csv(gene_expression_df_filename, row.names = TRUE, col.names = TRUE)


# Retrieve, from the vector of TFs of interest (created juste below), those for which I can retrieve both the activity scores and expression values
# TF_vector <- c("...", "...", "..."). Each element of the vector is a TF name, where only the first letter is in upper case.
get_tfs_interest_activity_expression(tf_results, gene_expression_df, TF_vector)
...
get_longitudinal_mean_expression_activity_tfs_drug_interest(tf_results, gene_expression_df, TF_vector, timepoint_vector, drug_interest, time_info)
...
# n_timepoints <- "..."
plot_longitudinal_mean_expression_activity_drug_interest(tf_results, gene_expression_df, TF_vector, timepoint_vector, drug_interest, n_timepoints, time_info)
...
tf_results <- read.csv(decoupleR_final_results_matrix_filename, row.names = TRUE, col.names = TRUE)


# Load the normalized gene expression df, used to run the decouple function
# Format: genes (only first letter in upper case) as rows and samples as columns. Sample names need to contain time info, such as 3h (for hours) or 3d (for days).
# In the case, where you have samples (i.e. column names) with no timepoint, the user needs to rename those columns, by adding 0h or 0d, in the column names.
# gene_expression_df_filename <- "...csv"
gene_expression_df <- read.csv(gene_expression_df_filename, row.names = TRUE, col.names = TRUE)

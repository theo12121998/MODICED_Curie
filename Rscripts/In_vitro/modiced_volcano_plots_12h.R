...
tt_MTX_24 <- tt %>% filter(contrast == "MTX_12h_vs_Control_12h")
...
volcano_mtx_24 <- ggplot(tt_MTX_24, aes(x=logFC, y=-log10(adj.P.Val), label=genes)) +
             ...
             ="dashed") + ggtitle("MTX vs CTRL - 12h") + theme_bw() +theme(plot.title = element_text(hjust=0.5,face="bold")) + theme(axis.title = element_text(face="bold")) + theme(axis.text = element_text(face="bold")) + labs(x = "log2FoldChange", y = "-log10(FDR)")

ggsave("/home/theo/Documents/MODICED/volcano_mtx_12.pdf",volcano_mtx_24,width=9,height=6,device = cairo_pdf)


tt_OXA_24 <- tt %>% filter(contrast == "OXA_12h_vs_Control_12h")
...
volcano_oxa_24 <- ggplot(tt_OXA_24, aes(x=logFC, y=-log10(adj.P.Val), label=genes)) +
                        ...
                        geom_hline(yintercept = -log10(0.05), colour = "black", linetype
             ="dashed") + geom_vline(xintercept = c(-2,2), colour="black", linetype
             ="dashed") + ggtitle("OXA vs CTRL - 12h") + theme_bw() +theme(plot.title = element_text(hjust=0.5,face="bold")) + theme(axis.title = element_text(face="bold")) + theme(axis.text = element_text(face="bold")) + labs(x = "log2FoldChange", y = "-log10(FDR)")

ggsave("/home/theo/Documents/MODICED/volcano_oxa_12.pdf",volcano_oxa_24,width=9,height=6,device = cairo_pdf)





tt_CIS_24 <- tt %>% filter(contrast == "CIS_12h_vs_Control_12h")
...
volcano_cis_24 <- ggplot(tt_CIS_24, aes(x=logFC, y=-log10(adj.P.Val), label=genes)) +
                        ...
                        geom_hline(yintercept = -log10(0.05), colour = "black", linetype
             ="dashed") + geom_vline(xintercept = c(-2,2), colour="black", linetype
             ="dashed") + ggtitle("CIS vs CTRL - 12h") + theme_bw() +theme(plot.title = element_text(hjust=0.5,face="bold")) + theme(axis.title = element_text(face="bold")) + theme(axis.text = element_text(face="bold")) + labs(x = "log2FoldChange", y = "-log10(FDR)")



ggsave("/home/theo/Documents/MODICED/volcano_cis_12.pdf",volcano_cis_24,width=9,height=6,device = cairo_pdf)
